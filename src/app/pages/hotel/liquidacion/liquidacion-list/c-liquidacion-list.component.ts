...
export class CLiquidacionListComponent implements OnInit, OnDestroy {
  ...
  getExportarExcel() {
 console.log('getExportarExcel: ', this.listado);
    if (this.listado === undefined || this.listado === null) {
     ...
     const $getListar = this.serviceHotel.exportarexcelhotelLiqui(objeto)
     .subscribe({
       next: (rpta:any) => {
           this.setSpinner(false);
           this.utilitariosService.descargarExcel(rpta, 'LiquidacionHotel');
       },
       error:(err)=>{
           this.setSpinner(false);
           this.serviceSharedApp.messageToast()
