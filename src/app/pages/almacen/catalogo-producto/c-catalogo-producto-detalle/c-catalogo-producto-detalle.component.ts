...
@Component({
  selector: 'app-c-catalogo-producto-detalle',
  templateUrl: './c-catalogo-producto-detalle.component.html',
  styleUrls: ['./c-catalogo-producto-detalle.component.scss']
})
export class CCatalogoProductoDetalleComponent implements OnInit, OnDestroy {
  @Input() IA_data: any;
  ...
  frmComponentes!: FormGroup;
  listaComponentes: any[] = [];
  listaComponentesProducto: any[] = [];
  ...
  ngOnInit(): void {
    ...
    if (this.idprod > 0) {
      this.traerUno();
      this.getListarComponentesProducto();
    } else {
  ...
  createFormRegistro() {
    ...
    this.frmComponentes = this.fb.group({
  ...
  getSubFamilia(dato: any) {
    const $getSubFamilia = this.almacenService.listarSubFamilia(dato).subscribe({
      next: (rpta: any) => {
        ...
        this.frmComponentes.get('idsubfamilia')?.setValue(0);
      },
      error: (err) => {
        this.setSpinner(false);
        console.info('error : ', err);
  ...
  getListarComponentes() {
    this.setSpinner(true);
    this.mensajeSpinner = mensajesSpinner.msjRecuperaLista
    const $buscarProducto = this.almacenService.buscarProducto(this.frmComponentes.value)
      .subscribe({
        next: (rpta: any) => {
          this.setSpinner(false);
          console.log('rpta listaComponentes', rpta);
          this.listaComponentes = rpta
        },
        error: (err) => {
          this.setSpinner(false);
          this.serviceSharedApp.messageToast()
    ...
    this.$listSubcription.push($buscarProducto)
  }

   getListarComponentesProducto() {
    this.setSpinner(true);
    this.mensajeSpinner = mensajesSpinner.msjRecuperaLista
    const $buscarProducto = this.almacenService.buscarComponentesProducto(this.IA_data.idcodigo)
      .subscribe({
        next: (rpta: any) => {
          this.setSpinner(false);
          console.log('rpta listaComponentesProducto', rpta);
          this.listaComponentesProducto = rpta
        },
        error: (err) => {
          this.setSpinner(false);
          this.serviceSharedApp.messageToast()
  ...
  editarComponente(data: any) {
    console.log('editarComponente', data);
    const ref = this.dialogService.open(CmProductoComponenteComponent, {
      data: {idProdPadre: this.IA_data.idcodigo, ...data},
      header: data.codproducto + ' - ' + data.despro,
      closeOnEscape: false,
    ...
    ref.onClose.subscribe(() => {
      this.getListarComponentes();
      this.getListarComponentesProducto();
    });
  }

  async eliminarComponente(idprodcomponente: number, cantidad:number){
    const rpta = await this.serviceSharedApp.confirmDialog({ message: '¿Desea Eliminar el Registro?' });
    if (!rpta) return;

    this.setSpinner(true);
    this.mensajeSpinner = mensajesSpinner.msjRecuperaLista
    const objeto = {
      idprod: this.IA_data.idcodigo,
      idprodcomponente,
      cantidad,
      idusuario: constantesLocalStorage.idusuario,
    }
    const $buscarProducto = this.almacenService.eliminarComponenteProducto(objeto)
      .subscribe({
        next: (rpta: any) => {
          this.setSpinner(false);
          console.log('rpta eliminarComponenteProducto', rpta);
          this.getListarComponentesProducto();
        },
        error: (err) => {
          this.setSpinner(false);
