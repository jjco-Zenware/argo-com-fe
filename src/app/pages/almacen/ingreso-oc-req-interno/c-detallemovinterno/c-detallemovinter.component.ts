...
@Component({
  ...
  templateUrl: './c-detallemovinter.component.html',
  styleUrls: ['./c-detallemovinter.component.scss']
})
export class CDetalleMovInterComponent implements OnInit, OnDestroy{
  @Input() IA_data: any;
  ...
  createFormRegistro() {
    ...
    this.registerFormRegistro = this.formBuilder.group({
      ...
      idtipoproyecto: [{ value: 2, disabled: false }],
      ...
      fecentrega: [{value: this.serviceUtilitario.obtenerFechaFormateadoDMA(),disabled: false,}],
    ...
    listaProyectoTipo(){
      this.ordencompraService.tipoProyectoList().subscribe({
        next: (rpta: any) => {
          ...
          let lista = rpta.filter((x: any) => x.idtipoproyecto != 1);
        this.lstOrigen = lista;
        },
        error: (err) => {
        this.messageService.clear();
        this.messageService.add({
