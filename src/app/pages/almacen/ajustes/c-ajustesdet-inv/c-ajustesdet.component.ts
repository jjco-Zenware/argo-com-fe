...
@Component({
  selector: 'app-c-ajustesdet',
  templateUrl: './c-ajustesdet.component.html',
  styleUrls: ['./c-ajustesdet.component.scss']
})
export class CAjustesdetComponent implements OnInit, OnDestroy {
  @Input() IA_data: any;
      ...
      this.comprasService.obtenerItemsTabla(133).subscribe({
          next: (rpta: any) => {
            console.info('lstTipoMovimiento : ', rpta);
              this.lstTipoMovimiento = rpta.filter((x: { iditem: any; }) => x.iditem === 583 || x.iditem === 588);
          },
          error: (err) => {
          console.info('error : ', err);
          this.serviceSharedApp.messageToast()
