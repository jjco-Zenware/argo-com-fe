...
@Component({
  selector: 'app-c-guiadet',
  templateUrl: './c-guiadet.component.html',
  styleUrls: ['./c-guiadet.component.scss']
})
export class CGuiaRemisionComponent implements OnInit, OnDestroy{
  ...
  lstComprobante: any[] = [];
  stateOptions: any[] = [
    { label: 'Remitente', value: 'on' },
    { label: 'Transportista', value: 'off' }
  ];
  lstUniMedida: any[] = [
    { idmedida: 'KGM', desmedida: 'KILOGRAMOS' },
    { idmedida: 'TNE', desmedida: 'TONELADAS' }
  ];
lstUbigeo: any;
  ...
  createFormRegistro() {
    ...
    this.registerFormRegistro = this.formBuilder.group({
      ...
      value:[{ value: 'on', disabled: false }],
      direccion:[{ value: '', disabled: false }],
      idproveedor_original:[{ value: 0, disabled: false }],
      gre_peso_bruto_unidad_de_medida: [{ value: '', disabled: false }],
    });
  }
  ...
  onAccion(item: any) {
    ...
    if (objeto.length > 0) {
      this.messageService.add({severity: 'warn', summary: 'Aviso', detail: 'Existen Items sin Seleccionar...!' });
          return;
    }else{
      this.guardarOC2(item);
    }
  ...
  validarDatos():boolean{
    ...
    console.log('this.formValue...', this.registerFormRegistro.value);    
      ...
      if (this.registerFormRegistro.value.gre_ruc_emp_transporte === " " || this.registerFormRegistro.value.gre_ruc_emp_transporte === null)
      {
      ...
      if (!_error && (this.registerFormRegistro.value.nrodocumento === " " || this.registerFormRegistro.value.nrodocumento === null))
      {
          this.errorMensaje="Ingresar N° Documento del Destinatario...!";
          _error = true;
      }

      if (!_error && (this.registerFormRegistro.value.idproveedor === null || this.registerFormRegistro.value.idproveedor === ''))
      {
          this.errorMensaje="Seleccionar Destinatario...!";
          _error = true;
      }

       if (!_error && (this.registerFormRegistro.value.gre_punto_partida === " " || this.registerFormRegistro.value.gre_punto_partida === null))
        {
            this.errorMensaje="Ingresar Dirección de Partida...!";
            _error = true;
        }
     ...
     if (!_error && (this.registerFormRegistro.value.gre_punto_llegada === " " || this.registerFormRegistro.value.gre_punto_llegada === null))
        {
            this.errorMensaje="Ingresar Dirección de Llegada...!";
            _error = true;
        }
      ...
      if (!_error && (this.registerFormRegistro.value.gre_punto_de_llegada_ubigeo === '' && this.registerFormRegistro.value.gre_punto_de_llegada_ubigeo === null) )
        ...
        if (!_error && (this.registerFormRegistro.value.gre_peso_bruto_unidad_de_medida === null || this.registerFormRegistro.value.gre_peso_bruto_unidad_de_medida === 0))
        {
            this.errorMensaje="Ingresar Unidad de Medida...!";
            _error = true;
        }
      ...
      if (!_error && (this.registerFormRegistro.value.gre_marca_placa_unid_transporte === " " || this.registerFormRegistro.value.gre_marca_placa_unid_transporte === null))
       ...
       if (!_error && (this.registerFormRegistro.value.gre_conductor_numero_licencia === " " || this.registerFormRegistro.value.gre_conductor_numero_licencia === null))
        ...
        if (!_error && (this.registerFormRegistro.value.gre_conductor_documento_numero === null || this.registerFormRegistro.value.gre_conductor_documento_numero === ''))
        {
            this.errorMensaje="Ingresar N° Documento del Conductor...!";
            _error = true;
        }
      ...
      if (!_error && (this.registerFormRegistro.value.gre_conductor_apellidos === " " || this.registerFormRegistro.value.gre_conductor_apellidos === null))
        ...
        if (!_error && (this.registerFormRegistro.value.gre_conductor_numero_licencia.length < 9))
        {
            this.errorMensaje="N° Licencia Debe ser de 9 caracteres como Mínimo...!";
            _error = true;
        }

        if (!_error && (this.registerFormRegistro.value.gre_conductor_documento_numero.length < 8))
        {
            this.errorMensaje="N° Documento Debe ser de 8 caracteres como Mínimo...!";
            _error = true;
        }
    ...
    getBusquedaAvanzada() {       
       ...
       const refItem = this.dialogService.open(CModalItemComponent, {
                  data: 0,
      ...
      validarAlmacen(valor:number){
        ...
        console.log('this.registerFormRegistro.value.alm_idalmacen_destino', this.registerFormRegistro.value.alm_idalmacen_destino);
        if (this.registerFormRegistro.value.idalmacen === this.registerFormRegistro.value.alm_idalmacen_destino) {
          this.messageService.add({severity: 'info', summary: 'Aviso', detail: "Debe Seleccionar Almacén de Salida" });
          if (valor === 1) {
            this.registerFormRegistro.get('alm_idalmacen_destino').setValue('');  
          }else{
            this.registerFormRegistro.get('idalmacen').setValue(''); 
          }
          return;
        }
...
listarItemsTablaSunat() {
  ...
  getDatos(dato:any, tipo:any){
      console.log('getDatos...', dato);
      let provee = this.lstCliente.filter((x: { idcliente: number; }) => x.idcliente === dato);
      if(tipo === 0){
        this.registerFormRegistro.get('gre_ruc_emp_transporte')?.setValue(provee[0].nrodocumento);          
        this.registerFormRegistro.get('gre_nom_emp_transporte')?.setValue(provee[0].razonsocial);
        this.registerFormRegistro.get('gre_punto_partida')?.setValue(provee[0].direcresumen);
      }else{
        this.registerFormRegistro.get('gre_punto_llegada')?.setValue(provee[0].direcresumen);
  ...
  listarUbigeos() {
    const $lstUbigeos = this.proyectosService.listarUbigeos().subscribe({
      next: (rpta: any) => {
        console.log('lstUbigeos', rpta);
        this.lstUbigeo = rpta;
      },
      error: (err) => {
        this.serviceSharedApp.messageToast()
    ...
    this.$listSubcription.push($lstUbigeos);
  }

   getBusquedaTransporte(){
    const _nro =  this.registerFormRegistro.get('gre_ruc_emp_transporte')?.value;
    ...
    this.ordencompraService.buscarporRUC(objet).subscribe({
      next: (rpta: any) => {
        this.setSpinner(false);     
        this.registerFormRegistro.get('idproveedor_original')?.setValue(rpta[0].idcliente);
      },
      error: (err) => {
        this.setSpinner(false);
      this.messageService.clear();
