...
export class CModalProductoComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.listaMonedas() ;
    this.getSubFamilia(125); // Cargar SubFamilia por defecto

    if (this.config.data !== undefined) {
      this.idprod = this.config.data.idprod;
      this.verbtnGrabar = this.config.data.verbtnGrabar;
      if (this.idprod > 0) {
  ...
  createFormRegistro() {
    ...
    this.registerFormRegistro = this.formBuilder.group({
      ...
      idfamilia:[{ value: 125, disabled: false }],
      ...
      indctrlunidad: [{ value: false, disabled: false }],
