...
export class CEventoDetalleComponent implements OnInit, OnDestroy {
    ...
    taskListOrigen: TaskList = { id: 0, title: '', tasks: [] };
    ...
    cargarData() {
        ...
        if (this.idCodigo > 0) {
            ...
            this.lstParticipantesext = this.IA_data.contactos.filter(
                (x: { tipocontacto: string }) => x.tipocontacto == 'E'
            );
            ...
            this.lstParticipantesextOrigen = this.IA_data.contactos.filter(
                (x: { tipocontacto: string }) => x.tipocontacto == 'E'
            );
    ...
    guardar() {
        ...
        if (
            this.registerForm.value.codcategoria === 410 ||
            this.registerForm.value.codcategoria === 416
        ) {
        ...
        if (this.idCodigo > 0) {
            if (startDate.toString().length === 10) {
                startDate = new Date(
                    this.serviceUtilitario.formatFecha(startDate)
                );
            }
        }
        ...
        let listaConcatena = this.lstParticipantes.concat(
            this.lstParticipantesext
        );
    ...
    validarDatos(): boolean {
        ...
        if (
            this.registerForm.value.codcategoria === '' ||
            this.registerForm.value.codcategoria === null
        ) {
        ...
        if (
            !_error &&
            (this.registerForm.value.codcategoria === 411 ||
                this.registerForm.value.codcategoria === 415 ||
                this.registerForm.value.codcategoria === 414)
        ) {
            if (
                this.registerForm.value.idcliente === '' ||
                this.registerForm.value.idcliente === null
            ) {
        ...
        if (
            ...
            (this.registerForm.value.title === '' ||
                this.registerForm.value.title === null)
        ) {
        ...
        if (
            !_error &&
            (this.registerForm.value.startDate === null ||
        ...
        if (
            !_error &&
            (this.registerForm.value.dueDate === null ||
                this.registerForm.value.dueDate === '')
        ) {
        ...
        if (
            !_error &&
            (this.registerForm.value.lugarevento === '' ||
                this.registerForm.value.lugarevento === null)
        ) {
        ...
        if (
            ...
            if (
                this.registerForm.value.idproveedor === '' ||
                this.registerForm.value.idproveedor === null
            ) {
        ...
        if (
            !_error &&
            (this.registerForm.value.idresponsable === '' ||
                this.registerForm.value.idresponsable === null)
        ) {
            this.errorMensaje = 'Seleccionar Responsable...!';
            _error = true;
        }
        if (
            (!_error && this.registerForm.value.idmoneda === null) ||
            this.registerForm.value.idmoneda === ''
        ) {
        ...
        if (
            !_error &&
            (this.registerForm.value.monto === null ||
                this.registerForm.value.monto === '' ||
                this.registerForm.value.monto === 0)
        ) {
    ...
    addTask(event: Event) {
        ...
        if (this.taskContent.trim().length > 10) {
            ...
            this.messageService.add({
                severity: 'info',
                detail: 'Debe Ingresar más de 10 Carácteres.',
            });
        }
    ...
    addTaskNew() {
        ...
        this.newTask = {
            idtarea: 0,
            sidtarea: '',
            text: 'Verificar los pendientes para el Evento...!',
            completed: false,
            fechafin: desdeStr,
            fechaini: desdeStr,
            nroorden: inroorden,
            asignados: [
                {
                    idasignado: constantesLocalStorage.idusuario,
    ...
    aceptarAsignado() {
        console.log(
            ' this.asignadosTareas.length....',
            this.asignadosTareas.length
        );
        if (this.asignadosTareas.length === 0) {
            this.messageService.add({
                severity: 'info',
                detail: 'Como mínimo la tarea debe tener un Asignado',
            });
            return;
        }
        for (let x = 0; x < this.taskList.tasks.length; x++) {
            if (
                this.taskList.tasks[x].idtarea == this.idtarea &&
                ...
                for (let z = 0; z < this.asignadosTareas.length; z++) {
                    this.taskList.tasks[x].asignados.unshift(
                        this.asignadosTareas[z]
                    ); //agregamos asignados a la tarea
                }
            }
        }
    ...
    agregarProveedor(data: any, index: number) {
        ...
        const refMensaje = this.dialogService.open(CModalProveedorComponent, {
            ...
            header:
                data.length == 0
                    ...
                    : 'Editar Participante - ' + data.nomcomercial, //'Selección de Cotización de ' +  data.nomcomercial,
        ...
        refMensaje.onClose.subscribe((rpta: any) => {
            ...
            if (rpta != undefined) {
                const _posAll: number = this.lstParticipantes.findIndex(
                    (x: { nroindex: number }) => x.nroindex == index
                );
                if (_posAll != -1) {
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                if (data.iditempostor > 0) {
                    const _posAll: number = this.lstParticipantes.findIndex(
                        (x: { iditempostor: any }) =>
                            x.iditempostor == data.iditempostor
                    );
                    if (_posAll != -1) {
                        this.lstParticipantes.splice(_posAll, 1);
                    }
    ...
    agregarProveedorExt(data: any, index: number) {
        ...
        const refMensaje = this.dialogService.open(CModalProveedorComponent, {
            ...
            header:
                data.length == 0
                    ...
                    : 'Editar Participante Externo- ' + data.nomcomercial, //'Selección de Cotización de ' +  data.nomcomercial,
            styleClass: 'testDialog',
            closeOnEscape: false,
            closable: true,
            width: '30%',
        ...
        refMensaje.onClose.subscribe((rpta: any) => {
            console.log('onClose index', index);
            if (rpta != undefined) {
                const _posAll: number = this.lstParticipantesext.findIndex(
                    (x: { nroindex: number }) => x.nroindex == index
                );
                if (_posAll != -1) {
    ...
    listaProveedores() {
        const $getClientes = this.marketingService
            .obtenerClientes('PRO')
            .subscribe({
                next: (rpta: any) => {
    ...
    listaClientes() {
        const $getClientes = this.marketingService
            ...
            .subscribe({
                next: (rpta: any) => {
                    this.lstClientes = rpta.filter(
                        (x: { idcliente: number }) => x.idcliente != 0
                    );
                },
                error: (err) => {
                    this.serviceSharedApp.messageToast();
    ...
    get mailtoLink() {
        return `mailto:${this.destinatario}?bcc=${
            this.cco
        }&subject=${encodeURIComponent(this.asunto)}&body=${encodeURIComponent(
            this.cuerpo
        )}`;
    }
    ...
    getListarConfirmados() {
        const $getClientes = this.marketingService
            .obtenerConfirmados(this.idCodigo)
            .subscribe({
                next: (rpta: any) => {
