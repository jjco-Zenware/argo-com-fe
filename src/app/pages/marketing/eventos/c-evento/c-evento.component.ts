...
@Component({
  ...
  lstProveedores: any[] = [];
    ...
    onVer(data: any) {
        this.tituloDetalle = "VER EVENTO" + data.razonsocial + "  RAZÓN SOCIAL: ";
  ...
  getKanbanList(){
    ...
    const $kanbanList = this.marketingService.kanbanEventoList(objeto)
        .subscribe({
            next: (rpta:any) => {
                this.setSpinner(false);
...
listaProveedores() {
  ...
  getExportarExcel() {
    ...
    const $getListar = this.marketingService.exportarExcelEvento(objeto)
    .subscribe({
      next: (rpta:any) => {
          this.setSpinner(false);
