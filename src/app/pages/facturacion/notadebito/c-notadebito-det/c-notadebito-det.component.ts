...
@Component({
  selector: 'app-c-notadebito-det',
  templateUrl: './c-notadebito-det.component.html',
  styleUrls: ['./c-notadebito-det.component.scss']
})
export class CNotaDebitoDetComponent implements OnInit, OnDestroy{
  @Input() IA_data: any;
...
createFormRegistro() {
  ...
  this.registerFormRegistro = this.formBuilder.group({
    ...
    idtipodocprc: [{ value: 20, disabled: false }],
    ...
    tipodoc_ctb:[{ value: 4, disabled: false }],
  ...
  traerUno(){
     ...
     const $cargarOrdenC = this.proyectosService.ordenCompraTraeruno(objeto)
       .subscribe({
         next: (rpta:any) => {
           console.log('rpta.ordencompra[0]', rpta);
  ...
  listarItemsTablaSunat() {
    ...
    listarItemsTablaTipoNota() {
      this.contabilidadService.listarItemsTablaSunat(5).subscribe({
        next: (rpta: any) => {
